...
#+begin_src emacs-lisp
  ...
  (defun low/capitalize-word-and-add-get-prefix ()
    "Capitalize the first letter of the word and add 'get' as a prefix."
    (interactive)
    (save-excursion
      (forward-word)
      (backward-word)
...
** Setting up Use Package and Org Mode
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (use-package hydra
    ...
    (defhydra hydra-window-width ()
      ...
      ("o" evil-window-decrease-width "decrease width")
  ...
  (use-package general
    ...
    (low/leader
      ...
      "eg" 'low/capitalize-word-and-add-get-prefix)
    (low/leader
...
#+begin_src emacs-lisp
    ...
    :commands lsp
    :hook (
            ((tsx-ts-mode typescript-ts-mode js-ts-mode js2-mode java-mode java-ts-mode svelte-mode) . lsp)
...
This makes emacs open terminal buffers in a side window (kinda like a popup) in the bottom of the screen.
#+begin_src emacs-lisp
  (add-to-list 'display-buffer-alist
               '("\\`\\*vterm\\*\\(?:<[[:digit:]]+>\\)?\\'"
...
Now, the following code block makes emacs open vterm inside normal mode instead of insert. This helps a lot, because evil-mode glitches and shows the wrong cursor char when opening vterm directly in insert mode (it shows the normal mode char but it's in insert mode).
#+begin_src emacs-lisp
  (setq evil-insert-state-modes (delq 'vterm-mode evil-insert-state-modes))
#+end_src
* UI
...
#+end_src
This is a function which gets the official tree-sitter grammar repo, builds the grammars and moves then to treesit-extra-load-path.
#+begin_src emacs-lisp
  (defun my/install-tree-sitter-modules ()
    "Clone casouri/tree-sitter-module, run 'JOBS=8 batch.sh',
  and move dist/* into the first directory in `treesit-extra-load-path`.
  If git clone or batch.sh fails, show the output buffer."
    (interactive)
    (unless treesit-extra-load-path
      (error "❌ `treesit-extra-load-path` is not set"))
    (let* ((repo-url "https://github.com/casouri/tree-sitter-module")
            (temp-dir (make-temp-file "tree-sitter-module-" t))
            (dist-dir (expand-file-name "dist" temp-dir))
            (dest-dir (car treesit-extra-load-path))
            (git-buffer "*Git Clone Output*")
            (batch-buffer "*Tree-sitter Batch Output*"))
      ;; Clone repo
      (message "Cloning %s..." repo-url)
      (unless (eq (apply #'call-process "git" nil git-buffer t
                    (list "clone" repo-url temp-dir))
                0)
        (pop-to-buffer git-buffer)
        (error "❌ Git clone failed — see %s" git-buffer))

      ;; Run JOBS=8 batch.sh
      (let ((default-directory temp-dir))
        (message "Running JOBS=8 batch.sh...")
        (unless (eq (apply #'call-process "bash" nil batch-buffer t
                      (list "-c" "JOBS=8 ./batch.sh"))
                  0)
          (pop-to-buffer batch-buffer)
          (error "❌ batch.sh failed — see %s" batch-buffer)))

      ;; Check dist/ exists
      (unless (file-directory-p dist-dir)
        (error "❌ dist directory not found inside repo"))
      ...
      (message "Moving generated files from %s to %s..." dist-dir dest-dir)
      (dolist (file (directory-files dist-dir t "^[^.].*"))
        (when (file-regular-p file)
          (rename-file file (expand-file-name (file-name-nondirectory file) dest-dir) t)))

      ;; Cleanup
      (delete-directory temp-dir t)
      (message "✅ Done! Tree-sitter modules installed to %s" dest-dir)))
#+end_src
** Emacs Lisp
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp :tangle no
  ...
  (setq lsp-bridge-jdtls-jvm-args '("-javaagent:/home/louhmmsb/.m2/repository/org/projectlombok/lombok/1.18.30/lombok-1.18.30.jar"))
#+end_src

* AI
The configuration below installs gptel (a simple client for LLMs inside emacs) and configures it to use Ollama as the default model. Ollama needs to be running on the specified host and port for it to work.
#+begin_src emacs-lisp
  (use-package gptel
    :straight t)
